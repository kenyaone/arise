...
if ($lessonType === 'interactive' && $filePath) {
    ...
    // Next module lookup (for end-of-lesson CTA)
    ...
    if ($_nm) {
        $nmSlug  = htmlspecialchars($_nm['slug'], ENT_QUOTES);
        $nmTitle = htmlspecialchars($_nm['title'], ENT_QUOTES);
        $nmIcon  = $_nm['icon'] ?: '📘';
        $nextModuleHtml = <<<HTML
<a href="/arise/?p=module&slug={$nmSlug}" style="margin-top:14px;display:flex;align-items:center;gap:12px;background:linear-gradient(135deg,#0a5e2a,#1a8a40);color:#fff;border-radius:14px;padding:14px 18px;text-decoration:none;box-shadow:0 6px 18px rgba(10,94,42,.3);">
  <span style="font-size:1.7rem;flex-shrink:0;">{$nmIcon}</span>
  <div style="flex:1;min-width:0;">
    <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;color:#fcd34d;margin-bottom:3px;">Next Module</div>
    <div style="font-size:.95rem;font-weight:800;color:#fff;line-height:1.3;">{$nmTitle}</div>
  </div>
  <span style="flex-shrink:0;background:rgba(255,255,255,.18);padding:8px 14px;border-radius:10px;font-size:.78rem;font-weight:900;white-space:nowrap;">Proceed &rarr;</span>
</a>
HTML;
    } else {
        $nextModuleHtml = '<div style="margin-top:14px;background:linear-gradient(135deg,#f5a623,#e8891a);border-radius:14px;padding:14px 18px;color:#fff;display:flex;align-items:center;gap:12px;box-shadow:0 6px 18px rgba(245,166,35,.3);"><span style="font-size:1.7rem;">🎉</span><div style="flex:1;"><div style="font-size:.95rem;font-weight:800;">You\'ve reached the final module!</div><div style="font-size:.78rem;opacity:.9;margin-top:2px;">Complete this one to finish the full ARISE journey.</div></div></div>';
    }
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    if(v){
      ...
      v.addEventListener('play',function(){
        ...
        // Persistent "Next Module" floating button (always visible at end of lesson)
        if ($_nm) {
            $nmSlugJs  = htmlspecialchars($_nm['slug'], ENT_QUOTES);
            $nmTitleJs = htmlspecialchars($_nm['title'], ENT_QUOTES);
            $nextFloatBtn = <<<HTML
<a id="arise-next-module-btn" href="/arise/?p=module&slug={$nmSlugJs}" title="Proceed to: {$nmTitleJs}"
   style="position:fixed;bottom:14px;right:14px;z-index:99998;display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,#0a5e2a,#1a8a40);color:#fff;border-radius:999px;padding:10px 18px;font-size:.85rem;font-weight:800;text-decoration:none;box-shadow:0 6px 20px rgba(10,94,42,.45);backdrop-filter:blur(4px);">
  <span>Next Module</span><span style="font-size:1.05rem;">&rarr;</span>
</a>
HTML;
        } else {
...
<div class="container">
    ...
    <div class="lesson-content">
        ...
        <div class="lesson-nav" style="display:flex;justify-content:space-between;margin-top:28px;padding-top:22px;border-top:1px solid var(--border);">
            ...
            <?php else: ?>
                <a href="/arise/?p=module&slug=<?= e($lesson['module_slug']) ?>" class="btn btn-primary">Finish Module &#8594;</a>
            <?php endif; ?>
        </div>

        <?php
        // Next module CTA — appears at end of every lesson
        $_curSo = db()->querySingle("SELECT sort_order FROM modules WHERE id=".intval($lesson['module_id']));
        $_nmStmt = db()->prepare('SELECT slug, title, icon FROM modules WHERE is_active = 1 AND sort_order > :so ORDER BY sort_order ASC LIMIT 1');
        $_nmStmt->bindValue(':so', intval($_curSo));
        $nextModule = $_nmStmt->execute()->fetchArray(SQLITE3_ASSOC) ?: null;
        ?>
        <?php if ($nextModule): ?>
        <a href="/arise/?p=module&slug=<?= e($nextModule['slug']) ?>"
           style="margin-top:20px;display:flex;align-items:center;gap:14px;background:linear-gradient(135deg,#0a5e2a,#1a8a40);color:#fff;border-radius:18px;padding:18px 22px;text-decoration:none;box-shadow:0 8px 24px rgba(10,94,42,.3);transition:transform .2s,box-shadow .2s;"
        ...
        <?php else: ?>
        <div style="margin-top:20px;background:linear-gradient(135deg,#f5a623,#e8891a);border-radius:18px;padding:18px 22px;color:#fff;display:flex;align-items:center;gap:14px;box-shadow:0 8px 24px rgba(245,166,35,.3);">
